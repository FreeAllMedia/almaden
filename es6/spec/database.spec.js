...
describe("Database(databaseConfig)", () => {
	...
	describe("(instance functions)", () => {
		...
		describe("(mocking)", () => {
			...
			describe(".unmock()", () => {
				it("should remove all mock queries", done => {
					database.unmock();
					database
						.select("*")
						.from("* something ### BOGUS")
						.results((error) => {
			...
			describe(".mock(queryValueMatrix)", () => {
				it("should allow regex keys for lookup", done => {
				...
				it("should return mock values for the designated queries", done => {
					database
						.select("*")
						.from("users")
						.where("id", 1)
				...
				it("should throw a designated error in place of results", done => {
					let expectedError = new Error("Bogus Error");
